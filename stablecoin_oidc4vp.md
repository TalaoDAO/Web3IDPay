...
1. [Overview](#overview)
   ...
   1.2 [Regulatory and Legal Context](#regulatory-and-legal-context)
   1.3 [Compliance Mapping](#compliance-mapping)
   1.4 [Use Cases](#use-cases)
   1.5 [Standards and Technologies Used](#standards-and-technologies-used)
2. [Payment Flow](#payment-flow)
   2.1 [Preconditions](#preconditions)
   2.2 [Flow Steps](#flow-steps)
3. [Authorization Request from Merchant](#authorization-request-from-merchant)
   3.1 [Minimal SD-JWT VC for Identity](#1-minimal-identity-sd-jwt-vc-pid-like)
   3.2 [SD-JWT VC for Blockchain Ownership](#2-sd-jwt-vc-for-blockchain-ownership)
   3.3 [OIDC4VP Authorization Request Overview](#oidc4vp-authorization-request-overview)
   3.4 [Transaction Data for Stablecoin Payment](#4-transaction-data-for-stablecoin-payment)
   3.5 [Presentation Definition](#5-presentation-definition)
4. [Response from Wallet](#response-from-wallet)
   4.1 [Presentation Submission](#presentation-submission)
   4.2 [VP Token](#vp_token)
   4.3 [Key Binding JWT](#key-binding-jwt)

## Overview

### Objectives

This document describes how **stablecoin payments** (e.g., USDC, DAI, or Tezos-based stable assets) can be seamlessly supported using an **EUDI Wallet** or an equivalent **non-custodial data wallet** that implements **OIDC4VP**, **Verifiable Credentials (VCs)**, and **digital asset transfer mechanisms**.  
...
- **Enable secure, privacy-preserving, and user-consented payments** with stablecoins across **multiple blockchain networks** (Ethereum, EVM-compatible chains, Tezos, etc.).  
- Use **cryptographic proofs** to verify blockchain address ownership in a **blockchain-agnostic** way.  
- Leverage **selective disclosure (SD-JWT)** to share only the minimum identity data required for **AML/KYC compliance**, ensuring **data minimization** as mandated by regulations like **GDPR**.  
...
This wallet profile leverages **OIDC4VP** and **SD-JWT** to ensure compliance with these frameworks while preserving user privacy through **selective disclosure**.
...
| ----------------------------- | ----------------------------------------------------- |
| Proof of wallet ownership   | **Blockchain ownership SD-JWT** (linked to tx_hash) |
| KYC / Identity verification | **Identity VC (PID/eID)** via SD-JWT                |
| Data minimization (GDPR)    | **Selective disclosure** with SD-JWT                |
...
- **JOSE and JWT Standards**
  ...
  - **[JWS – JSON Web Signature (RFC 7515)](https://www.rfc-editor.org/rfc/rfc7515)** – Used for signing SD-JWT payloads.
...
- **Blockchain and Token Standards**
  ...
  - **FA2 (Tezos)** – For Tezos-based stable tokens.
  - **Multi-chain Support** – Ethereum mainnet, EVM-compatible chains, Tezos, and others.
...
- Validates all **SD-JWTs**, the VPs, and `transaction_data_hash`.
...
This **SD-JWT VC is signed by the key of the blockchain address** of the wallet used to transfer the stable coins. The `cnf` is the wallet key of the Identity SD-JWT of the user. Claims are readable (no disclosures needed).
...
```json
{
  ...
  "wallet_address": "tz3RDC3Jdn4j15J7bBHZd29EUee9gVB1CxD9",
  "cnf": {
    "jwk": {
      "kty": "EC",
      "crv": "P-256",
...
`transaction_id` is an internal reference and not the on-chain tx_hash.
...
The wallet’s response **MUST be encrypted as a JWE (JSON Web Encryption)**.
The JWE `alg` header parameter **MUST** be `ECDH-ES` for key agreement using keys on the **P-256** curve, and the `enc` parameter **MUST** be `A128GCM` for symmetric encryption.
...
There are 2 SD-JWT in the vp_token array. The Identity SD-JWT must provide disclosures of `given_name`, `family_name` and `birth_date`. Each SD-JWT has its own Key Binding JWT according to the IETF [SD-JWT](https://www.ietf.org/archive/id/draft-ietf-oauth-selective-disclosure-jwt-22.html#name-key-binding-jwt) specifications.
...
Payload of the KB JWT attached to the Identity SD-JWT with `transaction_data_hashes` and `tx_hash`
...
3. **Selective Disclosure & Privacy**
   ...
   - **SD-JWT** enables the wallet to disclose only the required claims (e.g., name, birth date) while keeping other identity attributes hidden.
...
2. **Show Credential Disclosure Requirements**
   ...
   - Any **wallet ownership proofs** (e.g., blockchain ownership SD-JWT).
...
4. **Regulatory Proofs**
   ...
   - Signature verification results for the SD-JWT and KB-JWT components.
